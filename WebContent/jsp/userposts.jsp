			...
			<div class="col-sm-8 blog-main">
				...
				<ul id="pagination" class="pager">
					<li><c:if test="${currentPage > 1 }">
							<a href="userposts?current=1&authorid=${authorid}" class="btn"><span
								class="glyphicon glyphicon-step-backward" aria-hidden="true"></span></a>
							</a>
						</c:if></li>
					<li><c:if test="${currentPage ==1 }">
							<a href="userposts?current=1&authorid=${authorid}"
					...
					<li><c:if test="${currentPage > 1 }">
							<a
								href="userposts?current=${(currentPage-1)}&authorid=${authorid}"
								class="btn"><span class="glyphicon glyphicon-arrow-left"
					...
					<li><c:if test="${currentPage > 4 }">
							<span class="btn">...</span>
						</c:if></li>
					<li><c:if test="${currentPage > 3 }">
							<a
								href="userposts?current=${(currentPage-3)}&authorid=${authorid}"
								class="btn">${currentPage-3}</a>
						</c:if></li>
					<li><c:if test="${currentPage > 2 }">
							<a
								href="userposts?current=${(currentPage-2)}&authorid=${authorid}"
								class="btn">${currentPage-2}</a>
						</c:if></li>
					<li><c:if test="${currentPage > 1 }">
							<a
								href="userposts?current=${(currentPage-1)}&authorid=${authorid}"
								class="btn">${currentPage-1}</a>
						</c:if></li>
					<li><a href="#" class="btn active">${currentPage}</a></li>
					<li><c:if test="${lastPage - currentPage > 0 }">
							<a
								href="userposts?current=${(currentPage+1)}&authorid=${authorid}"
								class="btn">${currentPage+1}</a>
						</c:if></li>
					<li><c:if test="${lastPage - currentPage > 1 }">
							<a
								href="userposts?current=${(currentPage+2)}&authorid=${authorid}"
								class="btn">${currentPage+2}</a>
						</c:if></li>
					<li><c:if test="${lastPage - currentPage > 2 }">
							<a
								href="userposts?current=${(currentPage+3)}&authorid=${authorid}"
								class="btn">${currentPage+3}</a>
						</c:if></li>
					<li><c:if test="${lastPage - currentPage > 3 }">
							<span class="btn">...</span>
						</c:if></li>
					<li><c:if test="${lastPage - currentPage > 0 }">
							<a
								href="userposts?current=${(currentPage+1)}&authorid=${authorid}"
								class="btn"><span class="glyphicon glyphicon-arrow-right"
								aria-hidden="true"></span></a>
						</c:if></li>
					<li><c:if test="${lastPage - currentPage ==0 }">
							<a
					...
					<li><c:if test="${lastPage - currentPage > 0 }">
							<a href="userposts?current=${lastPage}&authorid=${authorid}"
								class="btn"><span class="glyphicon glyphicon-step-forward"
								aria-hidden="true"></span></a>
						</c:if></li>
					<li><c:if test="${lastPage - currentPage ==0 }">
							<a href="userposts?current=${lastPage}&authorid=${authorid}"
